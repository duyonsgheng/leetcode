package duys_code.day_35;

import java.util.PriorityQueue;

/**
 * @ClassName Code_10_
 * @Author Duys
 * @Description TODO
 * @Date 2021/12/10 13:33
 **/
public class Code_10_WalkToEnd {
    // 来自网易
// map[i][j] == 0，代表(i,j)是海洋，渡过的话代价是2
// map[i][j] == 1，代表(i,j)是陆地，渡过的话代价是1
// map[i][j] == 2，代表(i,j)是障碍，无法渡过
// 每一步上、下、左、右都能走，返回从左上角走到右下角最小代价是多少，如果无法到达返回-1
    public static int minCost(int[][] map) {
        if (map[0][0] == 2) {
            return -1;
        }
        PriorityQueue<Node> heap = new PriorityQueue<>((a, b) -> a.cost - b.cost);
        int n = map.length;
        int m = map[0].length;
        boolean[][] visited = new boolean[n][m];
        add(map, 0, 0, 0, heap, visited);
        while (!heap.isEmpty()) {
            Node cur = heap.poll();
            if (cur.row == n - 1 && cur.col == m - 1) {
                return cur.cost;
            }
            // 上下左右走
            add(map, cur.row - 1, cur.col, cur.cost, heap, visited);
            add(map, cur.row + 1, cur.col, cur.cost, heap, visited);
            add(map, cur.row, cur.col - 1, cur.cost, heap, visited);
            add(map, cur.row, cur.col + 1, cur.cost, heap, visited);
        }
        return -1;
    }

    public static void add(int[][] m, int i, int j, int pre, PriorityQueue<Node> heap, boolean[][] is) {
        if (i >= 0 && i <= m.length - 1 && j >= 0 && j <= m[0].length - 1 && m[i][j] != 2 && !is[i][j]) {
            heap.add(new Node(i, j, pre + m[i][j] == 0 ? 2 : 1));
            is[i][j] = true;
        }
    }

    public static class Node {
        public int row;
        public int col;
        public int cost;

        public Node(int a, int b, int c) {
            row = a;
            col = b;
            cost = c;
        }
    }
}
