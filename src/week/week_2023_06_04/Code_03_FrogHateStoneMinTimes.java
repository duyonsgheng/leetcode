...
/**
 * @author Mr.Du
 * @ClassName Code_03_FrogHateStoneMinTimes
 * @date 2023年06月29日
 */
// 来自华为社招笔试
...
// 可以直接通过
public class Code_03_FrogHateStoneMinTimes {
    // 这道题的思路很简单
    // 可以跳跃 3-5步，一个dp，当前来到i位置，
    // 如果不是石头，那么就是 min(dp[i-3],dp[i-4],d[i-5])
    // 如果是石头，那么就是 min(dp[i-3],dp[i-4],d[i-5])+1
    // 关键是距离很长，就需要做一个空间压缩
    public static int MAXN = 101; // 石头数量
    ...
    public static int reduce(int s, int t) {
        ...
        for (int i = 0; i < MAXK; i++) {
            for (int j = i + 1; j < Math.min(i + t + 1, MAXK); j++) {
                reach[j] = true;
            }
            if (!reach[i]) {
                cnt = 0;
