...
public class Code04_MinimumScoreAfterRemovals {
    ...
    public static int minimumScore(int[] nums, int[][] edges) {
        ...
        for (int i = 0, a, b, pre, pos, sum1, sum2, sum3; i < m; i++) {
            a = Math.max(dfn[edges[i][0]], dfn[edges[i][1]]); // 找dfn序比较大的。比较大的一定是下越下面
            for (int j = i + 1; j < m; j++) {
    ...
    // 当前来到u节点
    public static void f(int[] nums, List<List<Integer>> graph, int u) {
        int i = ++dfnCnt;
        dfn[u] = i;
        xor[i] = nums[u];
        size[i] = 1;
        for (int v : graph.get(u)) {
            if (dfn[v] != 0) { // 说明已经被处理过了，是当前节点的父节点
                continue;
            }
            f(nums, graph, v);
            xor[i] ^= xor[dfn[v]];
