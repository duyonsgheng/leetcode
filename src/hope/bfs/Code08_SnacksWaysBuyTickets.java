...
/**
 * @author Mr.Du
 * @ClassName Code08_SnacksWaysBuyTickets
 * @date 2023年11月24日 10:11
 */
// 牛牛的背包问题 & 世界冰球锦标赛
...
public class Code08_SnacksWaysBuyTickets {
    // 双向广搜
    // 由于零食的重量很大，不适用动态规划，但是零食的数量很小，可以暴力展开
    ...
    private static long compute() {
        int lsize = process(0, n >> 1, 0, w, lsum, 0);
        int rsize = process(n >> 1, n, 0, w, rsum, 0);
    ...
    // 展开arr[i.....e]
    // 返回ans 填到什么位置了
    private static int process(int i, int e, long s, long w, long[] ans, int j) {
        if (s > w) {
            return j;
        }
        // 最后一个了
        if (i == e) {
            ...
            // 不要i位置
            j = process(i + 1, e, s, w, ans, j);
            // 要i位置
            j = process(i + 1, e, s + arr[i], w, ans, j);
        }
        return j;
    }
}
