package hope.class82_enum1;

/**
 * @author Mr.Du
 * @ClassName Code03_Stock3
 * @date 2024年04月18日 上午 09:54
 */
// 买卖股票的最佳时机 III
// 给定一个数组，它的第 i 个元素是一支给定的股票在第 i 天的价格。
// 设计一个算法来计算你所能获取的最大利润。你最多可以完成 两笔 交易
// 注意：你不能同时参与多笔交易，你必须在再次购买前出售掉之前的股票
// 测试链接 : https://leetcode.cn/problems/best-time-to-buy-and-sell-stock-iii
public class Code03_Stock3 {
    // 不优化枚举
    public static int maxProfit1(int[] prices) {
        int n = prices.length;
        //dp1[i] 0...i范围上发生1次交易，不要求在i时刻卖出，最大利润
        int[] dp1 = new int[n];
        for (int i = 1, min = prices[0]; i < n; i++) {
            min = Math.min(min, prices[i]);
            dp1[i] = Math.max(dp1[i - 1], prices[i] - min);
        }
        // dp2[i] 0..i范围上发生两次交易，并且第二次交易在i时刻卖出，最大的利润
        int[] dp2 = new int[n];
        int ans = 0;
        for (int i = 1; i < n; i++) {
            // 第二次交易在i时刻卖出
            for (int j = 0; j <= i; j++) {
                // 枚举第二次买入的时机 j <= i
                dp2[i] = Math.max(dp2[i], dp1[j] + prices[i] - prices[j]);
            }
            ans = Math.max(ans, dp2[i]);
        }
        return ans;
    }

    //那么 dp2的答案 求dp2[5]
    //dp2[5] 依赖那些位置呢？
    //dp1[0] + arr[5]-arr[0] 意思是在一次交易中0到0位置上，和 第二次交易在 5位置卖出，0位置买入
    //dp1[1] + arr[5]-arr[1] 意思是在一次交易中0到1位置上，和 第二次交易在 5位置卖出，1位置买入
    //。。。。。。。。。
    //dp1[5] + arr[5]-arr[5] 意思是在一次交易中0到5位置上，和 第二次交易在 5位置卖出，5位置买入
    //那么移项，提取一下，就是 dp1[i]-arr[i] 的最大值 + 当前位置的arr[i] ，所以先生成一个辅助结构
    //就可以优化掉这个枚举行为。
    public static int maxProfit2(int[] prices) {
        int n = prices.length;
        //dp1[i] 0...i范围上发生1次交易，不要求在i时刻卖出，最大利润
        int[] dp1 = new int[n];
        for (int i = 1, min = prices[0]; i < n; i++) {
            min = Math.min(min, prices[i]);
            dp1[i] = Math.max(dp1[i - 1], prices[i] - min);
        }
        // best[i] 0...i范围上，所有的dp1[i]-prices[i]的最大值
        int[] best = new int[n];
        best[0] = dp1[0] - prices[0];
        for (int i = 1; i < n; i++) {
            best[i] = Math.max(best[i - 1], dp1[i] - prices[i]);
        }
        // dp2[i] 0..i范围上发生两次交易，并且第二次交易在i时刻卖出，最大的利润
        int[] dp2 = new int[n];
        int ans = 0;
        for (int i = 1; i < n; i++) {
            dp2[i] = best[i] + prices[i];
            ans = Math.max(ans, dp2[i]);
        }
        return ans;
    }

    // 对上面的方法进行等义转化
    public static int maxProfit3(int[] prices) {
        int n = prices.length;
        int[] dp1 = new int[n];
        int[] best = new int[n];
        best[0] = -prices[0];
        int[] dp2 = new int[n];
        int ans = 0;
        for (int i = 1, min = prices[0]; i < n; i++) {
            min = Math.min(min, prices[i]);
            dp1[i] = Math.max(dp1[i - 1], prices[i] - min);
            best[i] = Math.max(best[i - 1], dp1[i] - prices[i]);
            dp2[i] = best[i] + prices[i];
            ans = Math.max(ans, dp2[i]);
        }
        return ans;
    }

    // 空间压缩，使用几个变量滚动更新
    public static int maxProfit4(int[] prices) {
        int dp1 = 0;
        int best = -prices[0];
        int ans = 0;
        for (int i = 1, min = prices[0]; i < prices.length; i++) {
            min = Math.min(min, prices[i]);
            dp1 = Math.max(dp1, prices[i] - min);
            best = Math.max(best, dp1 - prices[i]);
            ans = Math.max(ans, best + prices[i]);
        }
        return ans;
    }
}
